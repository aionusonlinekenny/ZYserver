...
function showServerList(){
    $("#server_all").toggle();
    $(".mainbg, .announce, .shilin").toggle();
    $(".gameDiv").toggleClass('serverlist');
}
...
function updateWindowSize(showBox){
    if($(".gameDiv").length){
        ...
        //if(orien != 'portrait'){
        //    alert("请使用竖屏游戏更激情!手 游 源 码 网   w w w . s y y m w .c o m-");
        //    return false;
        //}
        ...
        if(showBox && $("#gbox").length){
            var _uniScale = Math.min(h/1649, w/750);
            var _topY,_leftX;
            var _scaleY = _uniScale;
            var _scaleX = _uniScale;
            _topY = (h - _uniScale * 1649) / 2;
            _leftX = (w - _uniScale * 750) / 2;
            $("#gbox").css({
                "width" : "750px",
                "height" : "1649px",
                "left" : _leftX + "px",
...
function updateWindowSize_h5(showBox){
    if($(".gameDiv").length){
        var frameWidth = $("body").width();
        var frameHeight = $("body").height();
        window.scrollTo(0, 1);
