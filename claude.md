...
- **Cập nhật sau khi người dùng deploy và test thật (2026-07-03)**: `lv.x=400` hết đè lên tên nhưng lại đè sang icon đồng tiền/giá nâng cấp (`costAll`, cùng hàng y≈15) bên phải — chỉnh lại `lv.x` **400→355** và `skillName.width` **275→230** để cân đối khoảng giữa (tên kết thúc ở x=345, còn cách lv 10px, lv kết thúc trước icon tiền). Đây là quy trình lặp thực tế do không thể xem trực tiếp UI đã deploy: sửa → người dùng deploy+chụp ảnh → tinh chỉnh tiếp theo phản hồi.
