...
local function GetCreateGuildNeedVip(actor,conf)
	local pf = LActor.getPf(actor)
	for i,v in ipairs(conf.pfVipLv or {}) do
		if v.pf == pf then
			return v.vipLv
		end
	end
...
function handleCreateGuild(actor, packet)
	...
	local needVip = GetCreateGuildNeedVip(actor, conf)
	if needVip ~= nil and LActor.getVipLevel(actor) < needVip then
...
function handleApplyJoin(actor, packet)
	...
	if isAuto == 1 then
		...
		if LGuild.getGuildMemberCount(guild) < getMaxMember(guild) then
			...
			local tips = string.format("欢迎[%s]加入仙盟，大家撒花、鼓掌", LActor.getName(actor))
...
function handleChangePos(actor, packet)
	...
	if pos == smGuildLeader then -- 禅让
		...
		local tips = string.format("%s禅让盟主给%s", LActor.getName(actor), name)
		guildchat.sendNotice(guild, tips)
	...
	elseif pos == smGuildAssistLeader then -- 任命副会长
		...
		local tips = string.format("任命%s为仙盟副盟主", name)
		guildchat.sendNotice(guild, tips)
	else -- 降职
		if LGuild.getGuildPos(guild, targetId) ~= smGuildAssistLeader then
			log_actor(actor, "guild pos error")
...
function handleDonateBonFire(actor, packet)
	...
	LActor.costItem(actor, GuildConfig.bonfireItem, needCount, "仙山")

	local reward = LActor.getRewardByTimes(GuildConfig.bonfireReward, needCount)
	LActor.giveAwards(actor, reward, "仙山")
