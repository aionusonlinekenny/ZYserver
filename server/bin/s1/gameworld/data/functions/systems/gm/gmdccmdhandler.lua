...
local function onLogin(actor)
	...
	if sysVar.paylist == nil then
		sysVar.paylist = {}
	end
	for i=#sysVar.paylist,1,-1 do
		local v = sysVar.paylist[i]
		if v then 
			if actor_id == v.actorid then
				table.remove(sysVar.paylist, i)
				doPay(actor,actor_id, v.yuanbao, v.ordernum, v.paytype)
			end
		end
	end
end
...
function doPay(actor,actor_id,yb,order_num,pay_type)
	local level = LActor.getLevel(actor)
	local var =  LActor.getStaticVar(actor)
	var.lastpaytype = pay_type
	var.lastpaycount = yb
	var.lastpayorder = order_num
	if yb == MonthCardConfig.monthCardMoney then
		var.firstmonthcard = nil
		monthcard.buyMonth(actor_id)
		System.writeLocalLog(qlPay,actor,level,0,order_num)
	elseif yb == PrivilegeData.priviMoney then 
		var.firstprivilege = nil
        privilegemonthcard.buyPrivilegeMonth(actor_id)
        System.writeLocalLog(qlPay,actor,level,0,order_num)
	else
		LActor.addRechargeOffline(actor_id, yb, 0, order_num,pay_type)
	end
end

gmDcCmdHandlers.daycharge = function (dp)
	local type = tonumber(LDataPack.readString(dp))
	daychargegift.dayChargeGiftGmOp(type)
end


--运营活动
gmDcCmdHandlers.activity = function(dp)
	local act_type =  LDataPack.readString(dp)  --活动类型
	local act_id   =  LDataPack.readString(dp)  --活动ID
	local act_conf  = LDataPack.readString(dp)  --活动基础配置
	local act_content = LDataPack.readString(dp) --活动内容
	activitysystem.GmSetActivity(act_type,act_id, act_conf, act_content)
end


-- 充值
gmDcCmdHandlers.pay = function (dp)
	local actor_id = tonumber(LDataPack.readString(dp))
	local yb  = tonumber(LDataPack.readString(dp))
	local order_num = LDataPack.readString(dp)
	local pay_type =  LDataPack.readString(dp)
	if actor_id == nil or  yb == nil then
		print('#GMCMD# pay actorid or yb nil !!!')
		return
	end
	if pay_type == nil or pay_type == "" then
		pay_type = "android"
	end
	local actor = LActor.getActorById(actor_id)
	if actor then 
		doPay(actor,actor_id,yb,order_num,pay_type)
	else
		local sysVar = System.getStaticVar()
		if sysVar.paylist == nil then
			sysVar.paylist = {}
		end
		local item = {actorid = actor_id, yuanbao = yb , ordernum = order_num, paytype = pay_type}
		table.insert(sysVar.paylist, item)
	end
end
...
gmDcCmdHandlers.addnotice = function(dp)
    ...
    local times = math.floor((et - st)/  (interval))
...
--刷新屏蔽词
gmDcCmdHandlers.filternames = function(dp)
