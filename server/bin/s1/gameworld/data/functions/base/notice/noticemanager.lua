...
local function loadNotice()
	_G.notice = {}
	local noticeFilePath = string.format("%s/%s",System.getRuntimePath(),noticeFile)
	local f = io.open(noticeFilePath)
	if not f then return end
	for line in io.lines(noticeFilePath) do
...
local function saveNotice()
	local noticeFilePath = string.format("%s/%s",System.getRuntimePath(),noticeFile)
	local f = io.open(noticeFilePath, "w+")
	if nil == f then
		return
	end
...
--添加一条公告 手 游    源 码 网 w w w . s y y m w .c o m
