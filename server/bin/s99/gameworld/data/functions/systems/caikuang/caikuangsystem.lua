...
--记录事件 手 游 源 码  网 w    w w .  s y y m w .c o m
local recordEvent = {
	beRob = 1,  -- 被抢夺
	rob = 2,     -- 抢夺
	revenge = 3, --复仇
...
local function requestRefreshKuangLevel(actor, packet)
	...
	System.writeLocalLog(qlMineRefesh,actor,type)  
...
local function requestGetReward(actor, pack)
	...
	if isDouble then
		...
		System.writeLocalLog(qlMineDouble,actor)  
	end
	...
	LActor.giveAwards(actor, reward, "采矿 id:".. tostring(var.kuangId))
