...
function getBossDataBySrvId(srvId)
	local data = getGlobalData()
	for i,v in pairs(data.bossList or {}) do
...
local function createBossFb(conf, srvId)
	...
	if getBossDataBySrvId(srvId) then
		return true
	end
...
local function initBossDataFb(list)
	...
	--有N个游戏服连接就开N+1个副本
	local srvSize = #(list or {})
	local srvIndex = 1
	for i=1, #CrossBossConfig do
		if srvIndex <= srvSize then 
			if createBossFb(CrossBossConfig[i], list[srvIndex]) then
				srvIndex = srvIndex + 1
			end
		end
	end
