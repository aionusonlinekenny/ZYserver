...
--充值双倍记录重置手 游      源 码 网 w w w . s y y m w .c o m
...
local function onRecharge(actor, count,itemid, ordernum, paytype)
	--是否为首充
	if true == dailyrecharge.isFirstRecharge(actor) then return end

	local actorId = LActor.getActorId(actor)
    local config = getConfig(count,paytype)
    ...
    System.writeLocalLog(qlRecharge,actor, count)
end
